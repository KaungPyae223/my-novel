...
const LetterEnableDisable = ({
  ...
  isFanLettersEnabled: boolean;
  toggleFanLetters: () => void;
}) => {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
        ...
        <div
          ...
          className={`flex text-sm items-center gap-2 p-3 text-white rounded-lg cursor-pointer
              ${isFanLettersEnabled ? "bg-red-500" : "bg-gray-800"}
          `}
        >
          {isFanLettersEnabled ? (
            <>
      ...
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        ...
        <p className="text-sm text-yellow-700 mt-1">
          When fan letters are closed, existing letters will remain visible
          but no new letters can be sent.
        </p>
      </div>
    </div>
  );
